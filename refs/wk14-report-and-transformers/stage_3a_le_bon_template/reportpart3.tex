...
% ReLU activation function - simple text version that works everywhere
...
$K^{(r)}_t$ & $r$-th order kernel & $T_{H+1}$ & New layer contribution \\
...
\section{Finite-Width Corrections and Learning Regimes}
...
\subsection{Personal Contribution: Scaling Laws for Third-Order Corrections}

\begin{tcolorbox}[colback=theoremcolor!10,colframe=theoremcolor!60!black,title={\textbf{Main Result: Scaling of $\textcolor{kernelcolor}{K^{(3)}}$ with Network Depth}},enhanced,drop shadow]
...
\|\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}\| \sim \mathcal{O}(m^{-1/2}) \cdot \rho^H
\end{equation}
...
\subsection{Personal Contribution: Novel \ReLU Recursion Formula}

We present the \textbf{first complete derivation} of the recursive structure for $\textcolor{kernelcolor}{K^{(3)}}$ in \ReLU networks, leveraging the critical property that $\sigma''(\\ReLU) = 0$ almost everywhere.
...
We introduce a \textbf{revolutionary perspective} using the Hessian of the NTK to make finite-width computations tractable.
...
\nabla^2_{\theta} \textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta) = \sum_{\gamma} \textcolor{kernelcolor}{K^{(3)}}(x_\alpha, x_\beta, x_\gamma) \otimes \nabla_\theta f(x_\gamma)
\end{equation}

This perspective reduces computational complexity from $\mathcal{O}(m^3)$ to $\mathcal{O}(m^2)$ for practical implementations.
\end{tcolorbox}

\section{Mathematical Foundation: The Third-Order Kernel}
...
\subsection{Architecture and Fundamental Setup}
...
\begin{itemize}
\item \textbf{Forward propagation}: $x^{(\ell)} = \frac{1}{\sqrt{m}}\sigma(W^{(\ell)}x^{(\ell-1)})$ for $\ell \in \{1, \ldots, H\}$
...
\frac{d}{dt} f(x, \theta_t) = -\frac{1}{n}\sum_{\beta=1}^n \textcolor{kernelcolor}{K^{(2)}}_t(x, x_\beta) (f(x_\beta, \theta_t) - y_\beta)
\end{equation}

where $\textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta) := \langle \nabla_\theta f(x_\alpha, \theta_t), \nabla_\theta f(x_\beta, \theta_t) \rangle$.
\end{tcolorbox}

The kernel admits the \textbf{layer-wise decomposition}:
\begin{equation}
\label{eq:ntk_decomposition}
\textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta) = \langle x^{(H)}_\alpha, x^{(H)}_\beta \rangle + \sum_{\ell=1}^{H} \langle G^{(\ell)}_\alpha, G^{(\ell)}_\beta \rangle \langle x^{(\ell-1)}_\alpha, x^{(\ell-1)}_\beta \rangle
\end{equation}
...
\frac{d}{dt} \textcolor{kernelcolor}{K^{(r)}}_t(x_{\alpha_1}, \ldots, x_{\alpha_r}) = -\frac{1}{n} \sum_{\gamma=1}^n \textcolor{kernelcolor}{K^{(r+1)}}_t(x_{\alpha_1}, \ldots, x_{\alpha_r}, x_\gamma) (f_t(x_\gamma) - y_\gamma)
\end{equation}
...
\textcolor{kernelcolor}{K^{(r+1)}}_t(x_1, \ldots, x_{r+1}) := \langle \nabla_\theta \textcolor{kernelcolor}{K^{(r)}}_t(x_1, \ldots, x_r), \nabla_\theta f_t(x_{r+1}) \rangle
\end{equation}
\end{tcolorbox}

This hierarchy represents a \textbf{fundamental departure} from infinite-width theory. While infinite-width networks have \textit{constant} $\textcolor{kernelcolor}{K^{(2)}}$, finite-width networks exhibit \textbf{dynamic evolution} where the first-order correction $\textcolor{kernelcolor}{K^{(3)}}$ drives the time-dependence of the NTK itself.

\textbf{Physical Interpretation}: The hierarchy captures the \textit{recursive feedback} between network predictions and kernel evolution. Each higher-order kernel $\textcolor{kernelcolor}{K^{(r+1)}}$ encodes how the $r$-th order kernel responds to parameter updates, creating an intricate web of dependencies that governs finite-width training dynamics.

\section{Third-Order Kernel: Theory and Computation}

This section develops the \textbf{theoretical framework} and \textit{computational methods} for the third-order kernel $\textcolor{kernelcolor}{K^{(3)}}$, which governs finite-width corrections to neural network training dynamics.

\subsection{Fundamental Evolution Equation}
...
\frac{d}{dt} \textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta) = -\frac{1}{n} \sum_{\gamma=1}^n \textcolor{kernelcolor}{K^{(3)}}_t(x_\alpha, x_\beta, x_\gamma) (f_t(x_\gamma) - y_\gamma)
\end{equation}

This equation reveals that $\textcolor{kernelcolor}{K^{(3)}}$ acts as the \textit{driving force} behind finite-width training dynamics.
\end{tcolorbox}

\subsection{Personal Contribution: Hierarchical Decomposition}
...
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = K^{(3, \text{out})}_{\alpha\beta\gamma} + \sum_{\ell=1}^H \left( K^{(3, G)}_{\alpha\beta\gamma, \ell} + K^{(3, x)}_{\alpha\beta\gamma, \ell} \right)
\end{equation}
\end{tcolorbox}

The detailed mathematical derivation involving \textit{directional derivatives}, \textit{recursive formulas}, and the complete expansion of all terms is presented in \hyperref[sec:appendix_k3_derivation]{Appendix B.1}. This derivation demonstrates how the hierarchical structure of the neural network naturally gives rise to the complex but well-organized form of $\textcolor{kernelcolor}{K^{(3)}}$.


\section{Extended Formula for $\textcolor{kernelcolor}{K^{(3)}}$}

To obtain a complete expression for $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$, we expand the terms $\delta_\gamma x^{(p)}_\mu$ and $\delta_\gamma G^{(\ell)}_\mu$ using their recursive definitions. We assume a \ReLU activation function, so $\sigma''=0$ almost everywhere.
...
Substituting these expressions into equations (18), (19) and (20) yields the complete formula for $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$. It is a sum of $2H^2 + 2H$ terms.
\begin{align}
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = & \sum_{j=1}^{H} \left( \langle \mathcal{J}^{(H \to j)}_{\alpha} \mathcal{T}^{(j)}_{\gamma\alpha}, x^{(H)}_\beta \rangle + \langle x^{(H)}_\alpha, \mathcal{J}^{(H \to j)}_{\beta} \mathcal{T}^{(j)}_{\gamma\beta} \rangle \right) \nonumber \\
...
\section{Fully Expanded Non-Recursive Formula for $\textcolor{kernelcolor}{K^{(3)}}$}

Here, we present the complete, non-recursive expression for $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$. This formula is obtained by substituting the expanded expressions for the directional derivatives $\delta_\gamma x^{(p)}_\mu$ and $\delta_\gamma G^{(\ell)}_\mu$ into the decomposed formula for $\textcolor{kernelcolor}{K^{(3)}}$. This makes the dependency on the network parameters (weights $W^{(k)}$ and output vectors $a_t$) and activations at each layer fully explicit. We continue to assume a \ReLU activation function, which sets second derivatives of $\sigma$ to zero almost everywhere.
...
\begin{align}
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = & \nonumber \\
% K^(3,out) term
...
\section{Recursive Analysis of Network Depth}

To understand how the third-order kernel scales with network depth, we establish a \textbf{recursive relationship} that reveals the \textit{layer-by-layer contributions} to $\textcolor{kernelcolor}{K^{(3)}}$. This analysis is crucial for understanding the behavior of finite-width corrections as networks become deeper.

\begin{tcolorbox}[colback=theoremcolor!10,colframe=theoremcolor!60!black,title={\textbf{Theorem (Recursive Depth Formula for $\textcolor{kernelcolor}{K^{(3)}}$)}},enhanced,drop shadow]\label{thm:recursive_depth}
...
This document analyzes the behavior of the entries of the $\textcolor{kernelcolor}{K^{(3)}}$ tensor as a function of the network depth, denoted by $L$ (or $H$ in the code), for a fixed network width $m$. The analysis is based on the formulas from the Neural Tangent Hierarchy (NTH) framework. We aim to understand if the tensor entries decay as the depth increases, and specifically, if this decay is exponential.

\section{Scaling Analysis and Depth Dependence}
...
\subsection{Personal Contribution: Backward Propagation Stability}
...
\section{Main Results: Scaling Laws and Theoretical Contributions}
...
\subsection{Personal Contribution: Complete Scaling Law for $\textcolor{kernelcolor}{K^{(3)}}$}
...
\boxed{\|\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}\| = \mathcal{O}\left(\frac{1}{\sqrt{m}}\right) \cdot \rho^H \cdot \text{poly}(\log m)}
\end{equation}
...
\subsection{Personal Contribution: GOE Statistical Universality}
...
\boxed{\text{Eigenvalue Statistics of } \textcolor{kernelcolor}{K^{(3)}} \text{ follow GOE with Tracy-Widom fluctuations}}
\end{equation}
...
\subsection{Personal Contribution: Revolutionary \ReLU Recursion}
...
\subsection{Personal Contribution: Hessian-NTK Framework}
...
\subsection{Overall Scaling of $\textcolor{kernelcolor}{K^{(3)}}$}

The full expression for $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$ is a sum of terms involving scalar products of the quantities analyzed above. For example:
\begin{equation}
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = \langle \delta_\gamma x^{(L)}_\alpha, x^{(L)}_\beta \rangle + \langle x^{(L)}_\alpha, \delta_\gamma x^{(L)}_\beta \rangle + \dots
\end{equation}
...
\begin{equation}
    \textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} \sim O(L/\sqrt{m})
\end{equation}

\section{Conclusion on Depth Dependence}

Our analysis shows that the magnitude of the entries of the $\textcolor{kernelcolor}{K^{(3)}}$ tensor does \textbf{not} decay with depth $L$. Instead, it appears to grow linearly with $L$. The scaling factor is $1/\sqrt{m}$.
...
Therefore, for a fixed width $m$, we expect the values of $\textcolor{kernelcolor}{K^{(3)}}$ to increase with the depth $L$. There is no evidence of an exponential decay. The factor $1/\sqrt{m}$ controls the overall magnitude but does not introduce a decay with depth.
...
We now provide a more detailed analysis of the scaling of $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$ with respect to the network depth $H$ (denoted $L$ in some contexts) and width $m$. This analysis is based on the fully expanded formula and relies on standard assumptions in the study of wide neural networks.
...
We analyze the four main components of the fully expanded formula for $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$. For simplicity, we analyze one of the two symmetric terms in each summation.
...
\[
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = O(H/\sqrt{m}) + O(H^2/m) + O(H/\sqrt{m}) + O(H^2/\sqrt{m})
\]
...
\begin{itemize}
    ...
    \item \textbf{\texttt{kernel3(alpha, beta, gamma)}}: This is the main public method. It computes the final scalar value of $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}$ by assembling all the pieces. It mirrors the high-level decomposition:
    \[ \textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = K^{(3, \text{out})}_{\alpha\beta\gamma} + \sum_{\ell=1}^H \left( K^{(3, G)}_{\alpha\beta\gamma, \ell} + K^{(3, x)}_{\alpha\beta\gamma, \ell} \right) \]
...
\begin{enumerate}
    \item Define the second-order kernel, $\textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta)$, as a pure JAX function that depends on the network parameters $\theta$.
    ...
    \item The third-order kernel, $\textcolor{kernelcolor}{K^{(3)}}$, is defined as the inner product $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = \langle \nabla_\theta \textcolor{kernelcolor}{K^{(2)}}_{\alpha\beta}, \nabla_\theta f_\gamma \rangle$. Since $\nabla_\theta f_\gamma$ can also be computed automatically with \texttt{jax.grad}, the entire $\textcolor{kernelcolor}{K^{(3)}}$ tensor can be obtained without manually writing out its complex formula.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Simplicity and Robustness:} The code becomes much simpler, as we only need to define the base objects ($f$ and $\textcolor{kernelcolor}{K^{(2)}}$), not their derivatives. This drastically reduces the risk of implementation errors.
    \item \textbf{Extensibility:} This methodology would make it far easier to compute even higher-order kernels, such as $\textcolor{kernelcolor}{K^{(4)}}$, which would be practically infeasible to implement manually.
\end{itemize}
...
The script \texttt{kernel3\_vs\_depth\_finite\_empirical\_largeexperiments.py} is designed to empirically validate the theoretical scaling laws for $\textcolor{kernelcolor}{K^{(3)}}$ derived in the previous sections. The primary goal is to investigate how the magnitude of the $\textcolor{kernelcolor}{K^{(3)}}$ tensor entries depends on the network depth $H$ and width $M$.
...
\begin{enumerate}
    ...
    \item \textbf{Kernel Computation}: An instance of the \texttt{Kernel3Empirical} class is created with the network weights and the results from the forward pass. The script then computes the entire $N \times N \times N$ tensor for $\textcolor{kernelcolor}{K^{(3)}}$ by calling the \texttt{kernel3(i, j, k)} method for all unique triplets of indices, leveraging the tensor's symmetry.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{What is the Infinity Norm?} The infinity norm of the $\textcolor{kernelcolor}{K^{(3)}}$ tensor, denoted $\|\textcolor{kernelcolor}{K^{(3)}}\|_\infty$, is simply the largest absolute value among all its entries:
    \[ \|\textcolor{kernelcolor}{K^{(3)}}\|_\infty = \max_{\alpha, \beta, \gamma} |\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma}| \]
    This metric provides a simple way to quantify the overall magnitude of the kernel.

    \item \textbf{The Purpose of Scaling by Width $M$}: In the infinite-width limit ($M \to \infty$), the standard NTK theory shows that $\textcolor{kernelcolor}{K^{(3)}}$ vanishes, which is why the kernel dynamics are ignored. Our analysis for finite-width networks aims to characterize precisely \textit{how fast} it vanishes. The scaling factor is introduced to cancel out the width-dependent decay, thereby isolating the depth-dependent behavior.

    \item \textbf{Testing Theoretical Predictions}: Our detailed scaling analysis predicted that the dominant term in $\textcolor{kernelcolor}{K^{(3)}}$ scales as $O(H^2/\sqrt{M})$. To empirically test this hypothesis, one should analyze the quantity $\sqrt{M} \cdot \|\textcolor{kernelcolor}{K^{(3)}}\|_\infty$. If the theory is correct, this scaled value should converge to a limit that grows quadratically with depth ($H^2$) as $M$ becomes large.

    The experiment, by computing $M \cdot \|\textcolor{kernelcolor}{K^{(3)}}\|_\infty$, is implicitly testing an alternative hypothesis where the kernel entries scale as $O(1/M)$. If our $O(1/\sqrt{M})$ theory is correct, the quantity computed in the script should not converge but instead grow like $\sqrt{M}$. By observing which of these scaled quantities (or another) stabilizes as $M$ increases, the experiment can empirically determine the true scaling law, thus validating or falsifying different theoretical claims.
\end{itemize}

\subsection{Spectral Analysis of Tensor Slices}

The eigenvalue analysis provides a much deeper view into the structure of $\textcolor{kernelcolor}{K^{(3)}}$ than the infinity norm alone. The dynamics of the NTK are given by the equation:
\[ \frac{d}{dt} \textcolor{kernelcolor}{K^{(2)}}_t = -\frac{1}{n} \sum_{\gamma=1}^n (f_t(x_\gamma) - y_\gamma) \cdot \textcolor{kernelcolor}{K^{(3)}}_{t, \gamma} \]
where $\textcolor{kernelcolor}{K^{(3)}}_{t, \gamma}$ is the $N \times N$ matrix slice of the tensor for a fixed index $\gamma$. The evolution of $\textcolor{kernelcolor}{K^{(2)}}$ is thus a linear combination of these slice matrices.

\begin{itemize}
    \item \textbf{Physical Meaning of Eigenvalues}: The eigenvalues of a slice matrix $\textcolor{kernelcolor}{K^{(3)}}_\gamma$ characterize how an error at a single data point, $x_\gamma$, influences the geometry of the feature space (as captured by $\textcolor{kernelcolor}{K^{(2)}}$). An eigenvector of this matrix represents a direction in the input space, and its corresponding eigenvalue represents the magnitude and sign of the change in that direction. A large positive eigenvalue means that the kernel's value for pairs of points aligned with that eigenvector will increase, effectively pushing those representations further apart.

    \item \textbf{Mean and Standard Deviation}:
    \begin{itemize}
        \item \texttt{mean\_eigenvalues}: By averaging the eigenvalues of all slices, we obtain the "average" spectral signature of the $\textcolor{kernelcolor}{K^{(3)}}$ operator. This tells us if there is a systematic, data-independent drift in the NTK. For instance, a consistently negative smallest eigenvalue across all slices would imply that gradient descent tends to contract the feature space along a specific direction, regardless of which data point has a large error.
    ...
    \item \textbf{Asymptotic Behavior}: As with the infinity norm, the eigenvalues are scaled by the width $M$ to study their asymptotic behavior. The eigenvalues of a matrix with entries scaling as $\epsilon$ will also scale as $\epsilon$. Therefore, if the entries of $\textcolor{kernelcolor}{K^{(3)}}$ scale as $O(1/\sqrt{M})$, its eigenvalues should as well. The experiment provides the necessary data to check if the scaled spectrum converges to a stable, non-trivial limit as the width grows.
\end{itemize}
...
These values maximize the lower bound on $\lambda_{\min}$ for a fixed parameter budget $P$ and dataset size $N$. A more rigorous approach requires careful analysis of the higher-order corrections $\textcolor{kernelcolor}{K^{(3)}}$ and $\textcolor{kernelcolor}{K^{(4)}}$, which we explore in detail in the following sections.
...
\[
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} \sim \Order(H/\sqrt{m}) + \Order(H^2/m) + \Order(H/\sqrt{m}) + \Order(H^2/\sqrt{m})
\]
...
\begin{tcolorbox}[colback=theoremcolor!10,colframe=theoremcolor!60!black,title={\textbf{Theorem (Scaling of $\textcolor{kernelcolor}{K^{(3)}}$)}},enhanced,drop shadow]
The magnitude of the entries of the third-order kernel $\textcolor{kernelcolor}{K^{(3)}}$ scales with network depth $H$ and width $m$ as:
\begin{equation}
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} \sim \mathcal{O}\left(\frac{H^2}{\sqrt{m}}\right)
\end{equation}
\end{tcolorbox}
...
The primary output of the experiment is a plot of the infinity norm of $\textcolor{kernelcolor}{K^{(3)}}$ as a function of the network depth $L$.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/k3_inf_norm_vs_L_finite_M100_N8_D20.png}
    \caption{Infinity norm of the empirical $\textcolor{kernelcolor}{K^{(3)}}$ tensor versus network depth $L$, plotted on a log-log scale. The width is fixed at $M=100$. The observed trend confirms that the magnitude of the correction kernel grows with depth.}
    \label{fig:k3_norm_vs_l}
\end{figure}

The results, as shown in Figure \ref{fig:k3_norm_vs_l}, demonstrate a clear increase in the magnitude of $\textcolor{kernelcolor}{K^{(3)}}$ as the depth $L$ increases. While a precise fit to a quadratic curve would require more extensive simulations over a wider range of depths, the observed trend is strongly consistent with our theoretical prediction of $\textcolor{kernelcolor}{K^{(3)}} \sim \Order(L^2/\sqrt{m})$. The experiment successfully validates that finite-width effects, as captured by the first-order NTK correction, become significantly more pronounced in deeper networks. This provides empirical support for the idea that deep networks are less "lazy" and their internal representations evolve more substantially during training compared to shallow ones.


To understand the practical impact of the $\textcolor{kernelcolor}{K^{(3)}}$ correction, we analyze how its magnitude scales with the network depth $H$ and width $m$. This analysis reveals how the importance of the NTK's evolution changes with the network architecture.
...
Following the derivation of $\textcolor{kernelcolor}{K^{(3)}}$, a natural next step in analyzing the NTK correction $\Theta^{(1)}_\infty$ (Equation \ref{eq:theta_correction}) would be to perform a similar analysis for the fourth-order tensor, $O_4$. A direct, term-by-term derivation, analogous to the one performed for $\textcolor{kernelcolor}{K^{(3)}}$ in Section 3, is theoretically possible. Indeed, this was the first path we pursued, leading to an extremely lengthy and complex formula spanning over four pages of dense calculations. While this direct approach is exhaustive, the resulting expression is too unwieldy to provide clear insights into the scaling properties of $O_4$. The sheer complexity of the formula makes it nearly impossible to analyze and computationally impractical to implement.

This significant obstacle prompted a search for a more elegant and computationally efficient representation. A recent key insight, which we present here, is that $O_4$ is not an entirely new entity but is intrinsically linked to the NTK ($\textcolor{kernelcolor}{K^{(2)}}$) itself. Specifically, $O_4$ can be expressed precisely as the action of the Hessian of the NTK on the gradients of the network's output.

\begin{proposition}[$O_4$ as the NTK Hessian]
The fourth-order kernel $\textcolor{kernelcolor}{K^{(4)}} \equiv O_4$ is given by the second directional derivative of the NTK, $\textcolor{kernelcolor}{K^{(2)}}$, along the directions defined by the output gradients. For any four inputs $x_\alpha, x_\beta, x_\gamma, x_\delta$, we have:
\begin{equation}
O_4(x_\alpha, x_\beta, x_\gamma, x_\delta) = \left( \nabla_\theta f(x_\gamma) \right)^T \left( \nabla^2_\theta \textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta) \right) \left( \nabla_\theta f(x_\delta) \right)
\end{equation}
where $\nabla^2_\theta \textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta)$ is the Hessian of the scalar-valued function $\theta \mapsto \textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta)$.
\end{proposition}

\begin{proof}
The proof follows directly from the definitions of the kernel hierarchy. Let $L(\theta; x_\alpha, x_\beta) = \textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta)$.
By definition of $\textcolor{kernelcolor}{K^{(3)}}$,
\begin{equation}
\textcolor{kernelcolor}{K^{(3)}}(x_\alpha, x_\beta, x_\gamma) = \langle \nabla_\theta \textcolor{kernelcolor}{K^{(2)}}(x_\alpha, x_\beta), \nabla_\theta f(x_\gamma) \rangle = \langle \nabla_\theta L, \nabla_\theta f(x_\gamma) \rangle
\end{equation}
This is the directional derivative of $L$ in the direction of the vector $v_\gamma = \nabla_\theta f(x_\gamma)$. We can write this as $\textcolor{kernelcolor}{K^{(3)}}(x_\alpha, x_\beta, x_\gamma) = (\partial_{v_\gamma} L)(\theta)$.

Next, by definition of $\textcolor{kernelcolor}{K^{(4)}} \equiv O_4$,
\begin{equation}
O_4(x_\alpha, x_\beta, x_\gamma, x_\delta) = \langle \nabla_\theta \textcolor{kernelcolor}{K^{(3)}}(x_\alpha, x_\beta, x_\gamma), \nabla_\theta f(x_\delta) \rangle
\end{equation}
Substituting the expression for $\textcolor{kernelcolor}{K^{(3)}}$, we get:
\begin{equation}
...
\end{equation}
This is precisely the action of the Hessian operator of $L = \textcolor{kernelcolor}{K^{(2)}}$ on the direction vectors $v_\gamma$ and $v_\delta$, which proves the proposition.
\end{proof}
...
This report has detailed the Neural Tangent Hierarchy as a framework for analyzing the training dynamics of finite-width neural networks. We focused on the first-order correction kernel, $\textcolor{kernelcolor}{K^{(3)}}$, which governs the time evolution of the standard NTK, $\textcolor{kernelcolor}{K^{(2)}}$.

Our primary contributions are:
\begin{itemize}
    \item A complete, non-recursive formula for the $\textcolor{kernelcolor}{K^{(3)}}$ kernel, derived from first principles.
    \item A scaling analysis demonstrating that the magnitude of $\textcolor{kernelcolor}{K^{(3)}}$ grows quadratically with network depth ($H^2$) and is suppressed by the square root of the width ($\sqrt{m}$).
...
In the framework of the Neural Tangent Hierarchy (NTH), the kernel $\textcolor{kernelcolor}{K^{(3)}}$ arises as the operator governing the time evolution of the Neural Tangent Kernel $\textcolor{kernelcolor}{K^{(2)}}$. Here we present the complete mathematical derivation.

\subsubsection{Foundation and Definition}

The $\textcolor{kernelcolor}{K^{(3)}}$ kernel is defined as the contraction of the gradient of $\textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta)$ with respect to the network parameters $\theta$ with the gradient of the network output $f_t(x_\gamma)$:
\begin{equation}
\textcolor{kernelcolor}{K^{(3)}}_t(x_\alpha, x_\beta, x_\gamma) := \langle \nabla_\theta \textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta), \nabla_\theta f_t(x_\gamma) \rangle.
\end{equation}

We start from the expression for $\textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta)$:
\begin{equation}
\textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta) = \langle x^{(H)}_\alpha, x^{(H)}_\beta \rangle + \sum_{\ell=1}^{H} \langle G^{(\ell)}_\alpha, G^{(\ell)}_\beta \rangle \langle x^{(\ell-1)}_\alpha, x^{(\ell-1)}_\beta \rangle
\end{equation}
...
\subsubsection{Directional Derivative Calculation}

Let $\delta_\gamma(\cdot) := \langle \nabla_\theta (\cdot), \nabla_\theta f_t(x_\gamma) \rangle$ denote the directional derivative along $\nabla_\theta f_t(x_\gamma)$. Then $\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = \delta_\gamma(\textcolor{kernelcolor}{K^{(2)}}_{\alpha\beta})$. Applying the product rule, we get:
\begin{align}
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = \delta_\gamma \langle x^{(H)}_\alpha, x^{(H)}_\beta \rangle + \sum_{\ell=1}^{H} \delta_\gamma \left( \langle G^{(\ell)}_\alpha, G^{(\ell)}_\beta \rangle \langle x^{(\ell-1)}_\alpha, x^{(\ell-1)}_\beta \rangle \right)
\end{align}

Each term expands further, following $\delta_\gamma \langle A, B \rangle = \langle \delta_\gamma A, B \rangle + \langle A, \delta_\gamma B \rangle$. This results in a sum over all components of $\textcolor{kernelcolor}{K^{(2)}}_{\alpha\beta}$, where each component is acted upon by $\delta_\gamma$. The core of the derivation lies in computing $\delta_\gamma$ for the building blocks $x^{(p)}_\mu$ and $G^{(p)}_\mu$.

\subsubsection{Recursive Formulas}
...
Combining these rules gives the complete formula for $\textcolor{kernelcolor}{K^{(3)}}_t(x_\alpha, x_\beta, x_\gamma)$. It is a sum of many terms, each corresponding to differentiating one part of $\textcolor{kernelcolor}{K^{(2)}}_t(x_\alpha, x_\beta)$. For clarity, we write it as a sum of contributions:
\begin{equation}
\textcolor{kernelcolor}{K^{(3)}}_{\alpha\beta\gamma} = K^{(3, \text{out})}_{\alpha\beta\gamma} + \sum_{\ell=1}^H \left( K^{(3, G)}_{\alpha\beta\gamma, \ell} + K^{(3, x)}_{\alpha\beta\gamma, \ell} \right)
\end{equation}
where
...
Here we present the complete derivation of the recursive formula for $\textcolor{kernelcolor}{K^{(3)}}$ as a function of network depth $H$.

\subsubsection{Decomposition of the $(H+1)$-Layer NTK}

Let $K^{(3, H)}_{\alpha\beta\gamma}$ denote the kernel for a network with $H$ layers. To express $K^{(3, H+1)}_{\alpha\beta\gamma}$ in terms of quantities computed for a network with $H$ layers, we start from the decomposition of $K^{(2, H+1)}_{\alpha\beta}$:
\begin{equation}
...
\subsubsection{Application of Directional Derivatives}
...
\subsubsection{Expansion of Terms}
...
\subsubsection{\ReLU Simplification}
...
The proof of the scaling of $\textcolor{kernelcolor}{K^{(3)}}$ follows from the recursive analysis of the network architecture and the application of concentration inequalities for deep networks.
