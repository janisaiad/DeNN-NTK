...
\textbf{Chebyshev points} (or optimal interpolation nodes) \cite{driscoll2014chebfun} enable accurate derivative approximation from function values alone, making standard regression sufficient. For spherical domains $\mathbb{S}^{d-1}$, we use Chebyshev nodes for the azimuthal coordinate $\phi \in [-\pi, \pi]$:
$$\phi_k = \pi \cos\left(\frac{k\pi}{N}\right), \quad k = 0, 1, \ldots, N$$
...
\begin{definition}[Discrete Sobolev Operator]
\label{def:discrete_sobolev_operator}
For a set of sampling points $\{x_i\}_{i=1}^n \subset \mathbb{S}^{d-1}$ with associated quadrature weights $\{c_i\}_{i=1}^n$, the discrete Sobolev operator $\Pop_s \in \mathbb{R}^{n \times n}$ of order $s > 0$ is defined by:

\begin{tcolorbox}[colback=blue!5,colframe=blue!60!black,title=Matrix Definition]
\begin{equation}
\label{eq:discrete_sobolev_matrix}
[\Pop_s]_{ij} = \sum_{\ell=0}^{\infty} \sum_{p=1}^{N(\ell)} \boldsymbol{(1 + \sqrt{\ell(\ell + d - 2)})^s} \boldsymbol{Y_{\ell,p}(x_i)} \boldsymbol{Y_{\ell,p}(x_j)} c_j
\end{equation}
\end{tcolorbox}

where $Y_{\ell,p}$ are the spherical harmonics of degree $\ell$ and order $p$, and $N(\ell) = \binom{\ell + d - 1}{\ell} - \binom{\ell + d - 3}{\ell - 1}$ is the dimension of the space of spherical harmonics of degree $\ell$ in dimension $d$.

For \textbf{Chebyshev points}, we obtain the exact spectral representation:
\begin{equation}
\label{eq:chebyshev_sobolev_exact}
[\Pop_s]_{ij} = \boldsymbol{\delta_{ij} (1 + \sqrt{\ell_i(\ell_i + d - 2)})^s}
\end{equation}
where $\ell_i$ is the harmonic degree associated with point $x_i$.
\end{definition}
