...
For general depth $L$ networks at EOC with bi-homogeneous leaky ReLU activation $\phi(x) = a\cdot\text{id}(x) + b\cdot|x|$, where $a,b \in \RR$ are fixed parameters, the NTK kernel function is:
\[
K^{\infty}(\rho) = \left( \sum_{k=1}^L \color{blue}\Sigma_k(\rho) \prod_{k'=k+1}^L \color{red}\Sigma'_{k'}(\rho) \right) 
\]
where $\color{blue}\Sigma_k$ and $\color{red}\Sigma'_k$ are defined recursively through Gaussian expectations:

\begin{align*}
\rho_1(x_1,x_2) &= \frac{x_1^T x_2}{\|x_1\| \|x_2\|} \\
\color{blue}\Sigma_1(x_1,x_2) &= \|x_1\| \|x_2\| \rho_1(x_1,x_2) \\
\color{blue}\Sigma^{(k)}(x_1,x_2) &= \mathbb{E}_{f \sim \mathcal{N}(0, \color{blue}\Sigma^{(k-1)})} \left[ \phi(f(x_1)) \phi(f(x_2)) \right] + \sigma_w^2 \\
\color{blue}\Sigma_k(x_1,x_2) &= \|x_1\| \|x_2\| \rho_k(x_1,x_2) \\
\color{red}\Sigma'_k(x_1,x_2) &= \|x_1\| \|x_2\| \rho_k(x_1,x_2)
\end{align*}

for $k \in [2:L]$, where $\sigma = (a^2 + b^2)^{-\frac{1}{2}}$ at the Edge of Chaos initialization and $\phi$ is the activation function.

For 2-layer ReLU networks, this simplifies to:
\[
\tilde{K}(\rho) = \color{blue}\arccos(-\color{red}\rho\color{blue})\color{black} \cdot \color{blue}\rho\color{black} + \color{red}\sqrt{1-\rho^2}\color{black}
\]

\subsection{NTK Operator Structure and Spectral Analysis}

The Neural Tangent Kernel operator $K^{\infty}$ acts on the space of square-integrable functions $L^2(\mathbb{S}^{d-1})$ through the integral representation:
\[
(K^{\infty} f)(x) = \int_{\mathbb{S}^{d-1}} k(x,y)f(y)d\sigma(y)
\]
where $d\sigma$ is the uniform measure on the sphere. Due to rotational invariance, the kernel exhibits zonal symmetry:
\[
k(x,y) = \tilde{K}_L^{\infty}(\langle x,y \rangle)
\]
for some function $\tilde{K}_L^{\infty}: [-1,1] \to \mathbb{R}$. This zonal structure allows application of the Funk-Hecke formula:
\[
\int_{\mathbb{S}^{d-1}} \tilde{K}_L^{\infty}(\langle x,y \rangle)Y_{\ell,p}(y)d\sigma(y) = \lambda_\ell Y_{\ell,p}(x)
\]
where
\[
\lambda_\ell = |\mathbb{S}^{d-2}| \int_{-1}^1 \tilde{K}_L^{\infty}(t)P_\ell^{(\frac{d-3}{2},\frac{d-3}{2})}(t)(1-t^2)^{\frac{d-3}{2}}dt
\]
and $P_\ell^{(\alpha,\beta)}$ are the Jacobi polynomials.
...
\hline
Symmetry & The operator is symmetric positive definite by construction \\
\hline
Eigenfunctions & The eigenfunctions are spherical harmonics $Y_{\ell,p}$, forming a complete orthonormal basis \\
\hline
Eigenvalues & The eigenvalues $\mu_\ell$ exhibit polynomial decay: $\mu_\ell \sim \ell^{-d}$ \\
\hline
Multiplicity & The multiplicity of each eigenvalue $\mu_\ell$ is given by the dimension of the corresponding spherical harmonic space \\
\hline
\end{tabular}
\end{table}
