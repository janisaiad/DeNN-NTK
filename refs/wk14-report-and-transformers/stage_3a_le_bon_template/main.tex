...
\section{Theoretical Framework of the Neural Tangent Kernel}

\subsection{Introduction: Eigenvalue Scaling Laws and Learning Dynamics}
...
$$K^{\infty}(x_i, x_j) = \left\langle \frac{\partial f(\x_i; \theta)}{\partial \theta}, \frac{\partial f(\x_j; \theta)}{\partial \theta} \right\rangle = \sum_{k} \frac{\partial f(\x_i; \theta)}{\partial \theta_k} \frac{\partial f(\x_j; \theta)}{\partial \theta_k}$$

For L2 gradient descent with learning rate $\eta$, the dynamics are given by:

$$\frac{d}{dt}f_t(x) = -\eta \sum_{i=1}^n K^{\infty}(x,x_i)(f_t(x_i) - y_i)$$
The following theorem from Jacot et al. \cite{jacot2020neuraltangentkernelconvergence} establishes the convergence of neural networks to their linearized version in the infinite width limit, along with the recursive kernel formulation:

\begin{theorem}[Neural Tangent Kernel, Jacot et al. 2018]
Consider a neural network $f(\x;\theta)$ trained by gradient descent with infinitesimal learning rate. As the width of the hidden layers tends to infinity:

1) The network converges to a Gaussian process with kernel $K^{\infty}$, with dynamics:
$$\frac{\partial f_t(\x)}{\partial t} = -\eta \sum_{i=1}^n K^{\infty}(\x,\x_i)(f_t(\x_i) - y_i)$$

2) The kernel follows the recursive formula for depth $l$:
$$K^{(l)}(\x,\x') = \sigma_w^2 \cdot K^{(l-1)}(\x,\x') \cdot \E_{g \sim \mathcal{N}(0,\Sigma^{(l-1)})} [\phi'(g(\x))\phi'(g(\x'))]$$
where $\Sigma^{(l)}$ is the covariance of pre-activations at layer $l$:
$$\Sigma^{(l)}(\x,\x') = \sigma_w^2 \cdot \E_{g \sim \mathcal{N}(0,\Sigma^{(l-1)})} [\phi(g(\x))\phi(g(\x'))]$$
with $\Sigma^{(0)}(\x,\x') = \x^\top\x'$ and $\phi$ the activation function.
...
where $\rho = \frac{\Sigma^{(l)}(\x,\x')}{\sqrt{\Sigma^{(l)}(\x,\x)\Sigma^{(l)}(\x',\x')}}$
\end{theorem}
...
Consider a dataset of $n$ points $x_1, x_2, \ldots, x_n \in \RR^d$. The NTK matrix has entries:
\[
K^{\infty} = D \cdot R \cdot D = \begin{pmatrix} 
\|x_1\| & 0 & \cdots & 0 \\
0 & \|x_2\| & \cdots & 0 \\
\vdots & \vdots & \ddots & \vdots \\
0 & 0 & \cdots & \|x_n\|
\end{pmatrix} \begin{pmatrix} 
\tilde{K}(\rho_{1,1}) & \tilde{K}(\rho_{1,2}) & \cdots & \tilde{K}(\rho_{1,n}) \\
\tilde{K}(\rho_{2,1}) & \tilde{K}(\rho_{2,2}) & \cdots & \tilde{K}(\rho_{2,n}) \\
\vdots & \vdots & \ddots & \vdots \\
\tilde{K}(\rho_{n,1}) & \tilde{K}(\rho_{n,2}) & \cdots & \tilde{K}(\rho_{n,n})
\end{pmatrix} \begin{pmatrix} 
\|x_1\| & 0 & \cdots & 0 \\
0 & \|x_2\| & \cdots & 0 \\
\vdots & \vdots & \ddots & \vdots \\
...
\end{pmatrix}
\]
where $\rho_{i,j} = \frac{x_i^T x_j}{\|x_i\| \|x_j\|}$ represents the cosine distance between points $x_i$ and $x_j$.
For general depth $L$ networks at EOC with bi-homogeneous leaky ReLU activation $\phi(x) = a\cdot\text{id}(x) + b\cdot|x|$, where $a,b \in \mathbb{R}$ are fixed parameters, the NTK kernel function is:
\[
\tilde{K}^{\infty}(\rho) = \left( \sum_{k=1}^L \varrho^{\circ (k-1)}(\rho) \prod_{k'=k+1}^L \varrho'(\varrho^{\circ (k'-2)}(\rho)) \right) 
\]
where $\varrho$ and $\varrho'$ are defined recursively through Gaussian expectations:
\begin{align*}
\rho_1(x_1,x_2) &= \frac{x_1^T x_2}{\|x_1\| \|x_2\|} \\
\varrho(\rho) &= \mathbb{E}_{[u_1,u_2] \sim \mathcal{N}(0,\Sigma_u)} \ReLU(u_1) \ReLU(u_2) \text{ where } \Sigma_u = \begin{pmatrix} 1 & \rho \\ \rho & 1 \end{pmatrix} \\
\varrho'(\rho) &= \mathbb{E}_{[u_1,u_2] \sim \mathcal{N}(0,\Sigma_u)} \ReLU'(u_1) \ReLU'(u_2) \text{ where } \Sigma_u = \begin{pmatrix} 1 & \rho \\ \rho & 1 \end{pmatrix} \\
\varrho^{\circ k}(\rho) &= \underbrace{\varrho(\varrho(\cdots \varrho}_{k \text{ times}}(\rho) \cdots))
\end{align*}

for $k \in [2:L]$, where $\sigma^2 = (a^2 + b^2)$ at the Edge of Chaos initialization and $\phi$ is the activation function.
...
\subsection{NTK Operator Structure and Spectral Analysis}

The Neural Tangent Kernel operator $K^{\infty}$ acts on $L^2(\mathbb{S}^{d-1})$ via:
\[
...
\[
(K^{\infty} f)(x) = \omega_{d-2} \int_{-1}^1 \tilde{K}^{\infty}(t) \bar{f}(t) (1-t^2)^{(d-3)/2} dt
\]
\[
= \int_{\mathbb{S}^{d-1}} K^{\infty}(x,y)f(y)d\sigma(y) = \omega_{d-2} \int_{-1}^1 \tilde{K}^{\infty}(t) \bar{f}(t) (1-t^2)^{(d-3)/2} dt
\]
where $\omega_{d-2}$ is the surface area of $\mathbb{S}^{d-2}$ and $\bar{f}(t)$ represents the average of $f$ over directions making angle $\arccos(t)$ with $x$.

\textbf{Spectral Properties and Funk-Hecke Theory}: The NTK operator possesses several important spectral properties that characterize its behavior on the sphere. The operator is symmetric positive definite by construction, guaranteeing the existence of a complete spectral decomposition with real, non-negative eigenvalues. The eigenfunctions are the spherical harmonics $Y_{\ell,p}$, which form a complete orthonormal basis of $L^2(\mathbb{S}^{d-1})$ and provide the natural coordinate system for analyzing functions on the sphere. Due to the zonal symmetry, the Funk-Hecke theorem applies, yielding eigenvalues $\mu_\ell$ given by:
\[
\mu_\ell = \frac{2\pi^{d/2}}{\Gamma(d/2)} \int_{-1}^1 \tilde{K}^{\infty}(t)P_\ell^{((d-2)/2)}(t)(1-t^2)^{(d-3)/2}dt
\]
where $P_\ell^{(\alpha)}$ are the Gegenbauer polynomials. The eigenvalues $\mu_\ell$ exhibit polynomial decay characterized by $\mu_\ell \sim \ell^{-d}$, reflecting the smoothness and regularity properties of the kernel function. The multiplicity of each eigenvalue $\mu_\ell$ is given by the dimension of the corresponding spherical harmonic space, which grows as $N(d,\ell) \sim \frac{2\ell^{d-2}}{(d-2)!}$ for large $\ell$, indicating the increasing complexity of higher-order harmonics.
\subsection{NTK-Sobolev Operator Framework}

The NTK-Sobolev framework integrates the Neural Tangent Kernel with Sobolev training by constructing a composite operator that incorporates high-order derivative information. The key innovation lies in modifying the standard $L^2$ loss to penalize the Sobolev norm, thereby controlling the smoothness of learned functions on the sphere $\mathbb{S}^{d-1}$.

\textbf{Definition of the Sobolev Operator}: The Sobolev operator $P_s$ acts on $L^2(\mathbb{S}^{d-1})$ through its spectral representation:
\[ P_s = \sum_{\ell=0}^{\infty} \sum_{p=1}^{N(d,\ell)} (1+\ell)^{2s} P_{\ell,p} \]
where $P_{\ell,p}$ denotes the orthogonal projection onto the span of the spherical harmonic $Y_{\ell,p}$, and $s > 0$ controls the strength of the Sobolev regularization.

\textbf{Discrete Matrix Formulation}: For data points $\{x_i\}_{i=1}^n \subset \mathbb{S}^{d-1}$, the discrete Sobolev operator matrix has entries:
\[ (P_s)_{ij} = \sum_{\ell=0}^{\ell_{\max}} \sum_{p=1}^{N(d,\ell)} (1+\ell)^{2s} Y_{\ell,p}(x_i) Y_{\ell,p}(x_j) \]
where $\ell_{\max}$ is chosen to ensure adequate spectral resolution.

\subsection{Sobolev Loss Formulation and Spectral Decomposition}

The Sobolev loss functional extends the standard $L^2$ norm by incorporating gradient penalties:
\[ \mathcal{L}_s[f] = \int_{\mathbb{S}^{d-1}} f(x)^2 d\sigma(x) + \int_{\mathbb{S}^{d-1}} \|\nabla f(x)\|^2 d\sigma(x) \]

Through Fourier analysis on the sphere, this admits the spectral representation:
\[ \mathcal{L}_s[f] = \sum_{\ell=0}^{\infty} \sum_{p=1}^{N(d,\ell)} (1+\ell(\ell+d-2))^s |\hat{f}_{\ell,p}|^2 \]
where $\hat{f}_{\ell,p} = \int_{\mathbb{S}^{d-1}} f(x) Y_{\ell,p}(x) d\sigma(x)$ are the spherical harmonic coefficients.

For discrete data, the coefficients are approximated as:
\[ \hat{f}_{\ell,p} \approx \frac{1}{n} \sum_{i=1}^n f(x_i) Y_{\ell,p}(x_i) \]
yielding the matrix form $\mathcal{L}_s[f] \approx f^T P_s f$ for $f = (f(x_1), \ldots, f(x_n))^T$.

\subsection{Theoretical Properties and Spectral Analysis}

\begin{theorem}[Common Eigenfunction Structure]
The NTK operator $K^{\infty}$ and Sobolev operator $P_s$ on $\mathbb{S}^{d-1}$ share spherical harmonics $\{Y_{\ell,p}\}$ as common eigenfunctions. Specifically:
\begin{align}
K^{\infty} Y_{\ell,p} &= \mu_\ell Y_{\ell,p} \\
P_s Y_{\ell,p} &= (1+\ell)^{2s} Y_{\ell,p}
\end{align}
where $\mu_\ell$ are the NTK eigenvalues given by the Funk-Hecke integral.
\end{theorem}

\begin{theorem}[Operator Commutation]
The NTK and Sobolev operators commute on $L^2(\mathbb{S}^{d-1})$:
\[ [K^{\infty}, P_s] = K^{\infty} P_s - P_s K^{\infty} = 0 \]
This commutation property extends to their discrete matrix approximations under appropriate sampling conditions.
\end{theorem}

\textbf{Spectral Complexity}: The dimension growth $N(d,\ell) \sim \frac{2\ell^{d-2}}{(d-2)!}$ for large $\ell$ determines the computational complexity of exact Sobolev operator construction, necessitating finite-rank approximations in practice.

\subsection{Edge of Chaos Initialization for Deep Networks}

For deep MLPs with parameterized ReLU activation $\phi(s) = as + b|s|$ where $a,b \in \mathbb{R}$, the Edge of Chaos initialization prevents gradient explosion or vanishing across network depth.

\textbf{Critical Parameters}:
\begin{itemize}
\item Weight variance: $\sigma^2 = (a^2+b^2)^{-1}$
\item Activation ratio: $\Delta_\phi = \frac{b^2}{a^2+b^2}$ (standard ReLU: $\Delta_\phi = 1/2$)
\item Correlation map: $\varrho(\rho) = \rho + \Delta_\phi \frac{2}{\pi}\left( \sqrt{1-\rho^2} - \rho \arccos(\rho) \right)$
\end{itemize}

At Edge of Chaos, the correlation function remains depth-invariant: $\varrho^{\circ L}(\rho) = \rho$ for all $L \geq 1$, ensuring stable signal propagation through the network.
