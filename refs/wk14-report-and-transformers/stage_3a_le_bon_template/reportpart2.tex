...
\subsection{NTK for a Single-Layer MMFN}
...
\subsection{Recursion for Multi-Layer MMNNs}
...
\begin{theorem}[NTK Recursion for Multi-Layer MMNNs]
\label{thm:ntk_recursion}
For a multi-layer MMNN with $\ReLU$ activation, the NTK satisfies the recursive relation:
...
\begin{equation}
\label{eq:ntk_recursion}
\boxed{\Theta^{(\ell)}(x, x') = \Theta^{(\ell-1)}(x, x') \cdot \sigma_A^2 \dot{\Sigma}^{(\ell)}(x, x') + \Sigma^{(\ell)}(x, x') + \sigma_c^2}
\end{equation}
where $\dot{\Sigma}^{(\ell)}$ is the derivative of the NNGP kernel with respect to the pre-activation.
\end{tcolorbox}
\end{theorem}
...
\subsection{Outlook: Application to PINNs and an Analogy to Wavelets}
...
\begin{equation}
\boxed{S_{ik} = \frac{e^{\textcolor{red}{\mathbf{X_{ik}}}}}{Z_i}, \quad \text{where } Z_i = \sum_{l=1}^{P} e^{\textcolor{methodcolor}{\mathbf{X_{il}}}}}
\end{equation}
...
Thus, $\mathbb{E}[P(S_{ik})]$ is the expectation of a polynomial of the derivatives of $\log Z_i$, making exact analytical calculation \textbf{intractable}.

\subsubsection{Approximation Strategy: Saddle-Point Method}

We introduce a parameter $\beta$ (inverse temperature) to obtain $S_{ik}(\beta) = e^{\beta X_{ik}} / \sum_l e^{\beta X_{il}}$. Following the asymptotic approach, we factorize the partition function $Z_a$:

\begin{equation}
\boxed{Z_a = \sum_{k=1}^{P} e^{\beta G_{ak}} = e^{\textcolor{red}{\beta G_{a, \text{max}}}} \left( 1 + \sum_{k \neq \text{max}} e^{\textcolor{methodcolor}{\beta (G_{ak} - G_{a, \text{max}})}} \right)}
\end{equation}

We define $\delta_k = e^{\beta (G_{ak} - G_{a, \text{max}})}$ for $k \neq \text{max}$.

\subsubsection{Cumulant Expansion Framework}
...
&\approx \beta G_{a, \text{max}} + \textcolor{insightcolor}{\left(\sum_{k \neq \text{max}} \delta_k\right)} - \textcolor{k3color}{\frac{1}{2}\left(\sum_{k \neq \text{max}} \delta_k\right)^2} + \dots \label{eq:log_expansion}
\end{align}
...
&\approx \exp\left( - \textcolor{insightcolor}{\sum_{k \neq \text{max}} \delta_k} + \textcolor{k3color}{\frac{1}{2}\left(\sum_{k \neq \text{max}} \delta_k\right)^2} - \dots \right) \\
&\approx 1 - \textcolor{red}{\sum_{k \neq \text{max}} \delta_k} + \textcolor{methodcolor}{\left(\sum_{k \neq \text{max}} \delta_k\right)^2} + \dots \label{eq:dominant_softmax}
\end{align}

For a perturbative output ($l \neq \text{max}$):

\begin{equation}
\boxed{S_{al} = \exp(\beta G_{al} - \log(Z_a)) \approx \textcolor{red}{\delta_l} \left(1 - \textcolor{methodcolor}{\sum_{k \neq \text{max}} \delta_k} + \dots \right)} \label{eq:perturbative_softmax}
\end{equation}

\subsubsection{High-Dimensional Analysis}

In the high-dimensional regime, we consider the behavior of dot products between random embeddings. For two independent random vectors $\mathbf{X}, \mathbf{Y} \in \mathbb{R}^d$ with i.i.d. $\mathcal{N}(0,1)$ components, their dot product $Z = \mathbf{X} \cdot \mathbf{Y}$ follows $Z \sim \mathcal{N}(0, d)$.

The absolute value $|Z|$ follows the folded normal distribution. We obtain the asymptotic behavior:

\begin{equation}
\boxed{\mathbb{E}[|Z|] \sim \sqrt{\frac{2}{\pi d}}, \quad \text{Var}(|Z|) \sim \left(1 - \frac{2}{\pi}\right) \frac{1}{d} \approx \frac{0.36338}{d}} \label{eq:high_dim_concentration}
\end{equation}

This concentration property justifies rigorously the hypothesis that dot products between random embeddings are typically small and concentrated around zero, which is fundamental for the analysis of attention mechanisms in high-dimensional settings.

\subsubsection{Correlated Gaussian Matrix Generation}

For the correlated Gaussian matrix $\mathbf{G}$, we assume a separable covariance structure. If we vectorize the matrix by stacking its columns into an $n^2 \times 1$ vector $\mathbf{g} = \text{vec}(\mathbf{G})$, its covariance matrix is:

\begin{equation}
\boxed{\text{Cov}(\mathbf{g}) = \mathbb{E}[\mathbf{g} \mathbf{g}^T] = \boldsymbol{\rho} \otimes \boldsymbol{\rho}} \label{eq:kronecker_covariance}
\end{equation}

This implies that the covariance between elements $G_{ik}$ and $G_{jl}$ is:

\begin{equation}
\text{Cov}(G_{ik}, G_{jl}) = \textcolor{red}{\rho_{ij}} \textcolor{methodcolor}{\rho_{kl}} \label{eq:separable_covariance}
\end{equation}

The standard construction via Cholesky decomposition ensures this exact structure. Let $\boldsymbol{\rho} = \mathbf{L} \mathbf{L}^T$ be the Cholesky decomposition, and $\mathbf{Z}$ be an $n \times n$ matrix of i.i.d. standard normal variables. Then:

\begin{equation}
\boxed{\mathbf{G} = \mathbf{L} \mathbf{Z} \mathbf{L}^T} \label{eq:cholesky_construction}
\end{equation}

\subsubsection{Connection to Hermite Polynomials and Generating Functions}

For the expansion of complex functions $f(x)$ in terms of Hermite polynomials, we use:

\begin{equation}
f(x) = \sum_{n=0}^{\infty} c_n H_n(x), \quad c_n = \frac{1}{\sqrt{\pi} 2^n n!} \int_{-\infty}^{\infty} f(x) H_n(x) e^{-x^2} dx \label{eq:hermite_expansion}
\end{equation}

Using the generating function approach with $e^{2xt-t^2} = \sum_{n=0}^{\infty} \frac{H_n(x)}{n!} t^n$, we arrive at:

\begin{equation}
\boxed{\int_{-\infty}^{\infty} f(x) e^{-(x-t)^2} dx = \sqrt{\pi} \sum_{n=0}^{\infty} (2t)^n c_n} \label{eq:generating_function_relation}
\end{equation}

\subsubsection{Multivariate Gaussian Integrals}

For the multivariate case, we introduce the generating functional $Z[\mathbf{J}]$ depending on an external source field $\mathbf{J}$:

\begin{equation}
\boxed{Z[\mathbf{J}] = \int f(\mathbf{x}) \exp\left(-\frac{1}{2} \mathbf{x}^T \boldsymbol{\Sigma}^{-1} \mathbf{x} + \mathbf{J}^T \mathbf{x}\right) d^d x} \label{eq:generating_functional}
\end{equation}

The moments are obtained by functional differentiation with respect to $\mathbf{J}$, and the combinatorics are governed by Wick's theorem, leading to the Feynman diagram formalism for organizing the calculations.

\subsubsection{Numerical Integration Considerations}

For practical computations, we note that Monte Carlo methods with importance sampling remain the most robust approach. The error decreases as $1/\sqrt{N}$ regardless of dimension:

\begin{equation}
I = \int f(\mathbf{x}) p(\mathbf{x}) d^d x \approx \frac{1}{N} \sum_{i=1}^{N} f(\mathbf{x}_i), \quad \mathbf{x}_i \sim p(\mathbf{x}) \label{eq:monte_carlo}
\end{equation}

For integrals of the form involving Gaussian measures, specialized quadrature methods such as Gauss-Hermite quadrature are particularly effective for integrals of the form $\int_{-\infty}^{\infty} e^{-x^2} g(x) dx$.
...
\subsection{Detailed Calculations for Two Hidden Layer MMNN}
\label{app:mmnn}
...
\subsubsection{Calculation of the Expectation $\E[\text{ReLU}(X_1)\text{ReLU}(X_2)]$}
...
\textbf{Properties we use:}
\begin{enumerate}
    \item Chain rule for gradients across layers
    \item Law of Large Numbers for infinite-width convergence  
    \item Independence of random parameters across neurons
\end{enumerate}
