...
\author{Discussion with Gemini 2.5 Pro}
\date{\today}
...
An exact analytical calculation is considered \textbf{intractable}. The structure of the softmax is intrinsically linked to the logarithm of the partition function $Z_i$, because:
\[
...
\begin{description}
    ...
    A Taylor expansion of the integrand $(S_{ik})^k$ in the variables $X_{jl}$ around zero. This leads back to Wick's theorem, but the complexity of the derivatives of the denominator makes this approach quickly intractable.
...
Let $Z_0$ be a single standard normal random variable ($Z_0 \sim \mathcal{N}(0,1)$). Let $Z'$ be an $n \times n$ matrix of i.i.d. standard normal random variables, independent of $Z_0$. Let $R$ be an $n \times n$ matrix of parameters, where $|R_{ij}| \le 1$ for all $i,j$.

The matrix $G$ is constructed entry-wise as:
\[
G_{ij} = Z_0 \cdot R_{ij} + Z'_{ij} \sqrt{1 - R_{ij}^2}
\]
This is equivalent to the matrix form:
\[
G = Z_0 R + Z' \circ \sqrt{J - R \circ R}
\]
...
\[
\text{Var}(G_{ij}) = \mathbb{E}[(Z_0 R_{ij} + Z'_{ij} \sqrt{1-R_{ij}^2})^2] = R_{ij}^2 \mathbb{E}[Z_0^2] + (1-R_{ij}^2) \mathbb{E}[(Z'_{ij})^2] = R_{ij}^2 + (1-R_{ij}^2) = 1
\]
The covariance between two different elements $G_{ik}$ and $G_{jl}$ (where $(i,k) \neq (j,l)$) is:
\begin{align*}
\text{Cov}(G_{ik}, G_{jl}) &= \mathbb{E}\left[(Z_0 R_{ik} + Z'_{ik} \sqrt{1-R_{ik}^2})(Z_0 R_{jl} + Z'_{jl} \sqrt{1-R_{jl}^2})\right] \\
&= R_{ik} R_{jl} \mathbb{E}[Z_0^2] + \mathbb{E}[Z'_{ik}Z'_{jl}] \sqrt{(1-R_{ik}^2)(1-R_{jl}^2)} \\
&= R_{ik} R_{jl}
\end{align*}
since $\mathbb{E}[Z'_{ik}Z'_{jl}] = \delta_{ij}\delta_{kl}$ which is zero for different elements.

The resulting covariance structure is $\text{Cov}(G_{ik}, G_{jl}) = R_{ik} R_{jl}$. This is a rank-one structure, which is generally different from the separable Kronecker product structure $\text{Cov}(G_{ik}, G_{jl}) = \rho_{ij} \rho_{kl}$. The former depends on all four indices in a non-separable way, while the latter separates row and column correlations. Therefore, while the second construction is a valid model for a correlated matrix, it does not produce the specific Kronecker product covariance structure.
...
For modern NLP models, the sequence length $N$ can be very large. In this large-$N$ limit, direct computation is intractable, but powerful approximations can be made by leveraging tools from Random Matrix Theory (RMT).
...
This sparsity is crucial. The spectral properties (i.e., the distribution of eigenvalues) of large, sparse random matrices are well-understood. Similarly, RMT provides precise results for the spectra of matrices that are low-rank perturbations of random matrices (like the rank-one structure from the alternative generative model). By assuming a sparse or low-rank structure for the correlation, we can approximate the behavior of the system and compute the desired moments in the large-$N$ limit, turning an intractable problem into a solvable one.
...
\[
G_{ij}(z_0, \{z'_{kl}\}) = z_0 \cdot R_{ij} + z'_{ij} \sqrt{1 - R_{ij}^2}
\]
...
We now refine the model to explicitly use a reduced set of random variables, as the polynomial of interest, $P$, only depends on rows $a$ and $b$. This is a crucial simplification for analytical tractability.
...
For $j = 1, \dots, P$:
\begin{align*}
G_{aj} &= Z_0 \cdot R_{aj} + Z_{aj} \sqrt{1 - R_{aj}^2} \\
G_{bj} &= Z_0 \cdot R_{bj} + Z_{bj} \sqrt{1 - R_{bj}^2}
\end{align*}
This construction maintains the desired covariance structure where $\text{Cov}(G_{ik}, G_{jl}) = R_{ik}R_{jl}$ for $i,j \in \{a,b\}$, but it only requires integrating over the variables that actually affect the quantity of interest.

\subsection{Final Integral Expression}
The expectation $\mathbb{E}[P(S_{a, \cdot}, S_{b, \cdot})]$ can now be expressed as an integral over these $2P+1$ variables. Let $d\mu(z)$ remain the standard Gaussian measure. The final, complete, and more tractable integral is:
...
\[
S_{al} = \frac{\exp\left(\beta \left[z_0 R_{al} + z_{al} \sqrt{1-R_{al}^2}\right]\right)}{\sum_{k=1}^{P} \exp\left(\beta \left[z_0 R_{ak} + z_{ak} \sqrt{1-R_{ak}^2}\right]\right)}
\]
\[
S_{bl} = \frac{\exp\left(\beta \left[z_0 R_{bl} + z_{bl} \sqrt{1-R_{bl}^2}\right]\right)}{\sum_{k=1}^{P} \exp\left(\beta \left[z_0 R_{bk} + z_{bk} \sqrt{1-R_{bk}^2}\right]\right)}
\]
