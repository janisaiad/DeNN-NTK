...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{figures/Capture d'écran 2025-09-02 173731.png}
...
En recollant les approximations locales $\rightarrow$ globales
\end{center}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{figures/landscape_mmnn.png}
\caption{Landscape MMNN}
\end{figure}
...
\begin{equation}
\NTK^{L}(\mathbf{x}_i, \mathbf{x}_j) = \left\langle \frac{\partial f(\mathbf{x}_i; \boldsymbol{\theta_A})}{\partial \boldsymbol{\theta_A}}, \frac{\partial f(\mathbf{x}_j; \boldsymbol{\theta_A})}{\partial \boldsymbol{\theta_A}} \right\rangle
\end{equation}
...
!! Aleatoire par nature car RBF !! mais std en $\frac{1}{r}$ donc conditionnement + concentration bounds
\end{frame}
...
\begin{frame}{Deep Gaussian Process et formule recursive}
...
Intégrales et opérateurs de convolution\ Analyse point fixe reportée pour L grand
...
\textbf{Validation future 2 layers:} prédictions théoriques d'un training\\
Loss finale, courbures, comportement asymptotique, dependance en L etc ..

Extension future sur L couches, lyapunov, controle de l'alea
\end{frame}
...
• H. Yang: transformers, S. Zhang: MMNN\\
• NP Baskerville, G. Ben Arous \& S. Péché: fondements RMT\\
